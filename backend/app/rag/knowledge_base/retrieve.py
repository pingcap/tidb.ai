...
from app.rag.chat_config import get_default_embedding_model, KnowledgeGraphOption
...
class RetrievalConfig(BaseModel):
    ...
    knowledge_graph: Optional[KnowledgeGraphOption] = None
...
class KBRetrieveService:
    ...
    def __init__(
        ...
        if config.llm_id:
            self._llm = llm_repo.must_get_llm(db_session, config.llm_id)
        else:
            self._llm = llm_repo.must_get_default_llm(db_session)

        if config.fast_llm_id:
            self._fast_llm = llm_repo.must_get_llm(db_session, config.fast_llm_id)
        else:
            self._fast_llm = self._llm

        if config.reranker_id:
            self._reranker = 

        _llm = self.chat_engine_config.get_llama_llm(db_session)
        _fast_llm = self.chat_engine_config.get_fast_llama_llm(db_session)
        _fast_dspy_lm = self.chat_engine_config.get_fast_dspy_lm(db_session)

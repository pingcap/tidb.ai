...
export function getArrayItemSchema<
  ChildType extends z.ZodAny | z.ZodArray<any, any> = z.ZodAny,
> (schema: ChildType | z.ZodEffects<ChildType>): ZodAny | null {
  if (!schema) return null;
  if ('innerType' in schema._def) {
    return getArrayItemSchema(schema._def.innerType as ChildType);
  }

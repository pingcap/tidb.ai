...
import { RecursiveCharacterSplitterForm } from '@/app/(main)/(admin)/extensions/rag.splitter.langchain.recursive-character/config/form';
...
export default function Page () {
  const { value, onChange, submitError, onSubmit, submitting } = useExtensionConfig('default', RecursiveCharacterTextSplitter);
  const id = useId();

  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold">{RecursiveCharacterTextSplitter.displayName} | Configuration</h1>
      {submitError != null && <Alert variant="destructive">
        <AlertTitle>{getErrorMessage(submitError)}</AlertTitle>
      </Alert>}
      <RecursiveCharacterSplitterForm id={id} disabled={!value || submitting} options={value} onValuesChange={onChange} onParsedValuesChange={() => {}} onSubmit={onSubmit} />
